                             ...
                             (call-with-semaphore
                              ...
                              (thunk
                               (for ((to-expire (in-vector expire))
                                 ...
                                 (when (time>? time (hash-ref ttl (cdr to-expire)))
                                   (log (format "expiring stat record ~a, ttl ~a > ~a" (cdr to-expire)
                                                (time->string time time-iso8601)
                                                (time->string (car to-expire) time-iso8601)))
      ...
      (let* ((fields (firewall-refused-fields refused))
             ...
             (by-ip-value (begin0 (prometheus-increment! by-ip #:labels by-ip-labels)
                            (call-with-semaphore
                             sem
                             (thunk
                              (let ((time (add-duration (current-time) (current-ttl))))
