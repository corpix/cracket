...
(define (write-json* who x o jsnull enc)
  ...
  (let loop ((x x))
    (cond ((or (exact-integer? x) (inexact-rational? x)) (write x o))
          ...
          ((symbol? x) (write-json-string (symbol->string x)))
          ((list? x)
           (write-bytes #"[" o)
           (when (pair? x)
             (loop (car x))
          ...
          ((hash? x)
           ...
           (for (((k v) (in-hash x)))
             (when (symbol? k)
               (set! k (symbol->string k)))
             (unless (string? k)
               (raise-type-error who "string|symbol" k))
      ...
      (else
       (let loop ((l (list (read-one))))
         ...
         (cond
           ...
           (else (err "error while parsing a json ~a at ~a" what ch)))))))
  ;;
  (define (read-hash)
    (define (read-pair)
      (define k (read-json))
      (unless (string? k) (err "non-string value used for json object key ~V" k))
      (define ch (skip-whitespace))
      (unless (char=? #\: ch)
        (err "error while parsing a json object pair ~V" ch))
      (read-byte i)
      (cons k (read-json)))
  ...
  (define (bad-input (prefix #"") #:eof? (eof? #f))
    ...
    (if (or (and (eof-object? bstr) (equal? prefix #""))
        ...
        (err (format "bad input starting ~e"
                     (bytes-append prefix (if (eof-object? bstr)
                                              #""
                                              bstr))))))
  (read-json #t))
