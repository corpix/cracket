...
(define (transformers-flatten transformers)
  (for/fold ((acc null))
    ...
    (append acc (map (partial cons (transformer-name transformer))
                     (transformer-procs transformer)))))

(define (transformer-ref transformers name . default)
        ...
        (for ((phase (in-list (*phases*))))
          (for ((transformer (in-list transformers)))
            (let ((proc (cdr transformer)))
              (when (eq? phase (proc-phase proc)) ;; FIXME: compose whole phase into single walk iterator
   ...
   (list (test-suite "walk"
         ...
         (test-suite "go/transform-once"
                     ...
                     (parameterize ((*transformers* (make-transformers)))
                       (with-empty-env
                         (check-equal?
                          (go/transform-once (package main)
                                             (import github.com/pkg/errors))
                          '(#s(go:expr #s(go:package main))
                            #s(go:expr #s(go:imports (#s(go:import github.com/pkg/errors #f))))))
                         (check-equal?
                          (*registry*)
                          (make-hasheq
                           (list (cons
                                  ...
                                  (env
                                   'main
                                   (package 'main (seteq 'github.com/pkg/errors))
                                   (make-hasheq))))))
                         ...
                         (check-equal?
                          (go/transform-once (package main)
                                             (import fmt))
                          '(#s(go:expr #s(go:package main))
                            #s(go:expr #s(go:imports (#s(go:import fmt #f))))))
                         (check-equal?
                          (*registry*)
                          (make-hasheq
                           (list (cons
                                  ...
                                  (env
                                   'main
                                   (package 'main (seteq 'fmt 'github.com/pkg/errors))
                                   (make-hasheq))))))
