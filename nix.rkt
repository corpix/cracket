...
(define (nix-generate value
  ...
  (let* ((space " ")
         ...
         (expression (cond
                       ...
                       ((nix-path? value)
                        (string-join (map (lambda (part) (nix-generate part #:level level))
...
(define (package-source->nix package)
  (let* ((url (package-source-url (hash-ref package 'source)))
         (remote-path (path->string (url->path url)))
         (url-string (url->string (make-url
                                   ...
                                   (filter (lambda (element)
                                           ...
                                           (url-query url))
                                   (url-fragment url))))
         (downloader (cond ((string-suffix? remote-path ".zip") 'fetchzip)
                           ((string-suffix? remote-path ".git") 'fetchgit)))
         (target-path (assocv 'path (url-query url)))
         (checksum (hash-ref package 'checksum))
         (arguments (cond
                      ;; FIXME: git hash discovering should be parallel, there is a room for improvement
                      ((eq? downloader 'fetchzip)
                       `((url . ,url-string)
                         (hash . ,(zip-hash-discover url-string))
                         (stripRoot . #f)))
                      ((eq? downloader 'fetchgit)
                       `((url . ,url-string)
                         (rev . ,checksum)
                         (hash . ,(git-hash-discover url-string checksum))))
                      (else `((url . ,url-string)))))
         (nix-expr (nix-function-call downloader (list (make-hash arguments))))
         (nix-expr (if target-path
                       (nix-function-call
                        'builtins.toPath
                        (list (nix-string
                               (nix-path
                                `(,(nix-interpolation nix-expr)
                                  ,@(map nix-literal (string-split target-path "/")))))))
                       nix-expr)))
    nix-expr))
...
(define (package->nix package (packages (current-packages)))
  (let* ((init-packages-catalog
         ...
         (install (list "raco pkg install --binary --batch --auto --user base64" "\n"
                        "mkdir $out/bin" "\n"
    ...
    (nix-function
     ...
     (nix-with 'pkgs
               (nix-with 'lib
                         (nix-function-call
                          'stdenv.mkDerivation
                          (list (make-hash `((name . ,(hash-ref package 'name))
...
(current-packages (catalogs->packages))
(define test-package (hash-ref (current-packages) "base64"))
